...
MongoClient.connect(mongoUrl, { useNewUrlParser: true, useUnifiedTopology: true })
  ...
  app.get('/registered-events/:eventId', async (req, res) => {
    const { eventId } = req.params; // Extract the eventId from the route parameters

    console.log("Fetching registrations for eventId:", eventId); // Log the eventId

    try {
        const eventsCollection = db.collection('events'); // Access the events collection

        // Fetch the event by its eventId
        const event = await eventsCollection
            .findOne({ "_id": new ObjectId(eventId) }); // Find the event by eventId

        console.log("Event fetched for eventId:", event); // Log the event fetched by eventId
        ...
        // Return only the registeredUsers field
...
app.post('/add-event', async (req, res) => {
    const { eventName, registrationFee, eventDate, numPeople, department, location,image } = req.body;
...
app.post('/register-event', async (req, res) => {
    ...
    try {
        const eventsCollection = db.collection('events');
        const usersCollection = db.collection('users'); // Assuming user data is stored here
        ...
        const userDetail = { username: user.username, email: user.email }; // Prepare the dictionary with username and email
        ...
        if (event.registeredUsers.some((user) => user.username === username)) {
        ...
        // Add the userDetail to the registeredUsers array
        const updatedEvent = await eventsCollection.updateOne(
            { _id: new ObjectId(eventId) },
            {
                $push: { registeredUsers: userDetail }, // Add the userDetail dictionary to the array
